...
class CamusSequenceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        target_mask = np.expand_dims(lv_mask, axis=0) # Shape: (1, H, W)

        class_label = 0 if sample["visao"] == "2CH" else 1

        img_res = (img_res - np.min(img_res)) / (np.max(img_res) - np.min(img_res) + 1e-8)

        img_tensor = torch.from_numpy(img_res).float().unsqueeze(0) # (1, H, W)
        mask_tensor = torch.from_numpy(target_mask).float()         # (1, H, W)
        class_tensor = torch.tensor(class_label, dtype=torch.long)  # Inteiro para a CrossEntropyLoss
